...
class FinetuneSolverBase(ABC):
    ...
    def build_model(self) -> (nn.Module, Tokenizer):
        ...
        if hasattr(unwrapped_model, "get_trainable_params"):
            ...
            trainable_params = unwrapped_model.get_trainable_params()
    ...
    def setup_fsdp_sync(self, model: nn.Module, data_parallel: str, precision: str, grad_precision: Optional[str]) -> FSDP:
        ...
        #     self.logger.info(f"CHECK Params: {name} with shape {param.shape}")
    ...
    def train_one_epoch(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            # print("optimizer gradient reset")
            if is_gradient_accumulation_boundary:
                # print("Before GradNorm Clip")
                # for name, param in self.model.named_parameters():
                #     if param.grad is not None:
                #         if torch.isnan(param.grad).any() or torch.isinf(param.grad).any():
                #             print(f"Gradient of {name} contains NaN or Inf values.")
                grad_norm = self.model.clip_grad_norm_(max_norm=self.args.clip_grad)
                # print("After GradNorm Clip")
            ...
            if len(self.optimizer.param_groups) > 1:    # Set different learning rate for ViT
                vit_lr = self.optimizer.param_groups[0]["lr"]
                llm_lr = self.optimizer.param_groups[1]["lr"]
                if "vit_lr" not in metric_logger.meters.keys():
                    metric_logger.add_meter("vit_lr", misc.SmoothedValue(window_size=1, fmt="{value:.6f}"))
                metric_logger.update(vit_lr=vit_lr)
                metric_logger.update(lr=llm_lr)
            else:
