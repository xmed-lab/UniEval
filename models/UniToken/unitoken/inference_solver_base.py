...
class FlexARInferenceSolverBase:
    @classmethod
    ...
    def generate(
        ...
        if len(images) == 0 and '<|image|>' not in conversations[0]['value']:   # Text-only Inputs
            ...
            image = self.vit_processor(images=images[0], return_tensors="pt").pixel_values   # (1,3,384,384)
            with torch.no_grad():
                # vit_features = self.vit(image.to(self.model.device).to(self.dtype), interpolate_pos_encoding=True).last_hidden_state
                vit_features = self.model.vit(image.to(self.model.device).to(self.dtype), interpolate_pos_encoding=True).last_hidden_state
                continuous_tokens = self.model.adapter(vit_features).squeeze(0)
