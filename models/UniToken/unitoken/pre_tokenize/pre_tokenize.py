...
if __name__ == "__main__":
    ...
    for i in tqdm(range(start_idx, end_idx)):
        ...
        try:
            ...
            new_item = {"token": tokens, "label": labels, "id": i, "raw_image": ori_contents[i]['image']}
            try:
                with open(pkl_path, "wb") as f:
                    pickle.dump(new_item, f)
            except:
                # Update "save_dir" and "pkl_path" because last folder is full
                save_dir = os.path.join(output_dir, f"files_{sub_dir_id}")
                os.makedirs(save_dir, exist_ok=True)
                pkl_path = os.path.join(save_dir, f"{i}.pkl")
                with open(pkl_path, "wb") as f:
                    pickle.dump(new_item, f)
