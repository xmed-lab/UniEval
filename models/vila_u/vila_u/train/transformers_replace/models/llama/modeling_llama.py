...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if vision_tower and mm_projector:
                if input_ids[-1, -1] == self.vocab_size - 2 or input_ids[-1, -1] == self.vocab_size - 4:
                    inputs_embeds = self.embed_tokens(input_ids)
                else:
                    image_ids_end = image_ids[-1].clone()
                    if hasattr(vision_tower.vision_tower, "rqvaesiglip"):
                        inputs_embeds = vision_tower.vision_tower.rqtransformer.embed_with_model_aux(image_ids_end, vision_tower.vision_tower.rqvaesiglip)
                    else:
                        raise NotImplementedError()
                    inputs_embeds = torch.cumsum(inputs_embeds, dim=-2)[:,:,-1,:]
                    inputs_embeds = mm_projector(inputs_embeds)
            else:
                # print(input_ids)
                inputs_embeds = self.embed_tokens(input_ids)
    ...
    def forward(
        ...
        if vision_tower and mm_projector:
            ...
            if input_ids[-1, -1] == self.vocab_size - 2 or input_ids[-1, -1] == self.vocab_size - 4:
                ...
                if hasattr(vision_tower.vision_tower, "rqvaesiglip"):
                    image_hidden_state, code = vision_tower.vision_tower.rqtransformer.generate(hidden_state, vision_tower.vision_tower.rqvaesiglip, cfg)
                else:
                    raise NotImplementedError()

                image_hidden_state = mm_projector(image_hidden_state)
                hidden_states = torch.cat([hidden_states[:, :-1, :], image_hidden_state], dim=1)
                image_ids.append(code)
            else:
                if hasattr(vision_tower.vision_tower, "rqvaesiglip"):
                    image_hidden_state, code = vision_tower.vision_tower.rqtransformer.generate(hidden_states, vision_tower.vision_tower.rqvaesiglip, cfg)
                else:
                    raise NotImplementedError()
